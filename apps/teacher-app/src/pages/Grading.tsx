...
const Grading = () => {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div>
        <h1 className="page-title">Grading & Assessment</h1>
        <p className="page-subtitle mt-1">Review and update student performance</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        <div className="surface-card p-6">
          <p className="text-sm font-medium text-muted-foreground mb-2">Average Grade</p>
          <p className="text-4xl font-semibold tracking-tight">{stats.average || "—"}</p>
        </div>
        <div className="surface-card p-6">
          <p className="text-sm font-medium text-muted-foreground mb-2">Pending Reviews</p>
          <p className="text-4xl font-semibold tracking-tight text-destructive">{stats.pending}</p>
        </div>
        <div className="surface-card p-6">
          <p className="text-sm font-medium text-muted-foreground mb-2">Graded</p>
          <p className="text-4xl font-semibold tracking-tight">{stats.gradedThisWeek}</p>
        </div>
      </div>
          ...
          <TableBody>
            {filtered.map((row) => (
              <TableRow key={row.id}>
                ...
                <TableCell>
                  <Badge
                    variant={row.status === "graded" ? "default" : "secondary"}
                  ...
                  <Dialog>
                    ...
                    <DialogContent className="rounded-2xl">
                      ...
                      <div className="space-y-4">
                        ...
                        <Button className="rounded-full" onClick={() => void saveGrade(row.id)}>
                          Save grade
                        </Button>
                      </div>
                    </DialogContent>
